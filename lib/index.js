...
var buildRequestArray = function () {
	var ra = '[{"api_action":"lastUpdated"},{"api_action":"getLatestValue","keys":["SysInfo.hostname","SysInfo.cpu.type","SysInfo.os.*","CPU.*","Load.*","Memory.*","Uptime","Disk.*","Network.Interface.*"]},{"api_action":"getValues","keys":["Packages.*"]}]';
	return ra;
};
...
function buildLongview(d, flags) {
	var lv = {};
	if (flags.r) {
		lv = d;
		return lv;
	...
	// Packages
	if ({}.hasOwnProperty.call(d[2].DATA, 'Packages')) {
		lv.packageUpdates = d[2].DATA.Packages.length;
	}
	return lv;
}
...
module.exports = function (keys, flags, callback) {
	...
	if (flags.f) {
		...
		return callback('Error: Longview API key invalid - check config.json');
	}
	options.apiKeys.forEach(function (key) {
		calls.push(function (callback) {
			...
			request(requestOptions, function (error, response, body) {
				...
				longviews.push(buildLongview(JSON.parse(body), flags));
				callback();
			});
		});
	});
