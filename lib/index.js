...
var buildLongview = function (data, flags) {
	...
	lv.rxBytes = 0;
	lv.txBytes = 0;
	for (var key in d[1].DATA.Network.Interface){
...
var printLongview = function (longviews) {
	...
	longviews.forEach(function (lv) {
		...
		console.log('CPU: '.bold + lv.cpuType);
		console.log('CPU Usage: '.bold + lv.cpuTotal + '%' + ' Load: '.bold + lv.load);
		...
		console.log('Network In: '.bold + getBitsPerSec(lv.rxBytes) + ' Out: '.bold + getBitsPerSec(lv.txBytes));
	});
};
